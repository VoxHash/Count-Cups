...
class DemoFeedGenerator:
    ...
    def __init__(self, width: int = 640, height: int = 480):
        ...
        self.drinking_phase = 0  # 0: not drinking, 1: approaching, 2: drinking, 3: leaving
    ...
    def _draw_face(self, frame: np.ndarray, center: Tuple[int, int], size: int) -> None:
        ...
        # Face outline
        cv2.ellipse(frame, (x, y), (size, int(size * 1.2)), 0, 0, 360, (200, 180, 160), -1)
        ...
        # Mouth
        mouth_y = y + size // 3
        cv2.ellipse(frame, (x, mouth_y), (size // 4, size // 8), 0, 0, 180, (100, 50, 50), 2)
    ...
    def _draw_hand(self, frame: np.ndarray, center: Tuple[int, int], size: int) -> None:
        ...
        # Hand (skin color)
        cv2.ellipse(frame, (x, y), (size, int(size * 0.8)), 0, 0, 360, (200, 180, 160), -1)
        ...
        if self.drinking_phase == 2:
            cup_x = x + size
            cup_y = y
            cv2.rectangle(frame, (cup_x - 10, cup_y - 20), (cup_x + 10, cup_y + 20), (100, 100, 100), -1)
            cv2.rectangle(frame, (cup_x - 10, cup_y - 20), (cup_x + 10, cup_y + 20), (255, 255, 255), 2)
    ...
    def generate_detection_result(self, frame: np.ndarray) -> Optional[DetectionResult]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate demo video feed for Count-Cups")
        ...
        while True:
            ...
            if detector:
                ...
                if result and result.has_sip:
                    detection_count += 1
                    print(f"Detection #{detection_count}: confidence={result.confidence:.2f}")
