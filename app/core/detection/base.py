...
class HeuristicDetector(DetectionEngine):
    ...
    def __init__(self, **kwargs):
        ...
        self.hand_face_distance_threshold = kwargs.get('hand_face_distance_threshold', 100.0)
    ...
    def detect(self, frame: np.ndarray) -> Optional[DetectionResult]:
        ...
        has_sip = (
            ...
            abs(head_tilt_angle) > self.head_tilt_threshold
        )
    ...
    def _detect_hand_by_skin_color(self, frame: np.ndarray) -> Optional[tuple[int, int]]:
...
class MediaPipeDetector(DetectionEngine):
    ...
    def detect(self, frame: np.ndarray) -> Optional[DetectionResult]:
        ...
        if has_sip:
            confidence = min(1.0, 1.0 - (distance / 200.0))  # Higher confidence for closer hand
    ...
    def _calculate_head_tilt_mediapipe(self, face_landmarks, frame_shape: tuple) -> float:
    ...
    def is_available(self) -> bool:
        ...
        return (
            self.mp_hands is not None and 
            self.mp_face_mesh is not None and 
            self.hands is not None and 
            self.face_mesh is not None
        )
