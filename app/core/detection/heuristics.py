...
class AdvancedHeuristicDetector(HeuristicDetector):
    ...
    def detect(self, frame: np.ndarray) -> Optional[DetectionResult]:
        ...
        has_sip = (
            hand_face_distance < self.hand_face_distance_threshold and
            abs(head_tilt_angle) > self.head_tilt_threshold and
            motion_detected
        )
    ...
    def _detect_hand_advanced(self, frame: np.ndarray, face: tuple[int, int, int, int]) -> Optional[tuple[int, int]]:
    ...
    def _detect_hand_by_contours(self, frame: np.ndarray, face: tuple[int, int, int, int]) -> Optional[tuple[int, int]]:
    ...
    def _detect_hand_by_edges(self, frame: np.ndarray, face: tuple[int, int, int, int]) -> Optional[tuple[int, int]]:
    ...
    def _calculate_head_tilt_advanced(self, face: tuple[int, int, int, int], gray: np.ndarray) -> float:
    ...
    def _detect_motion_in_face_region(self, gray: np.ndarray, face: tuple[int, int, int, int]) -> bool:
    ...
    def _calculate_confidence(
        ...
        distance_factor = max(0, 1.0 - (hand_face_distance / self.hand_face_distance_threshold))
        ...
        duration_factor = 1.0 - abs(sip_duration - 2.0) / 2.0  # Optimal around 2 seconds
