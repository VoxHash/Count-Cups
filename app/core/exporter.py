...
from app.core.models import DailyStats, CupProfile, SipEvent, ExportData
...
class DataExporter:
    ...
    def export_daily_stats_csv(
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'date', 'total_ml', 'total_sips', 'total_cups', 'goal_ml',
                'goal_achieved', 'progress_percentage', 'streak_days'
            ]
    ...
    def export_sip_events_csv(
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'timestamp', 'profile_id', 'ml_estimate', 'source', 'confidence',
                'detection_data'
            ]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            ...
            for event in sip_events:
                writer.writerow({
                    ...
                    'detection_data': json.dumps(event.detection_data) if event.detection_data else ''
                })
    ...
    def export_cup_profiles_csv(
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'id', 'name', 'size_ml', 'sips_per_cup', 'color', 'is_default',
            ...
            for row in reader:
                ...
                if row['detection_data']:
                    try:
                        ...
                        logger.warning(f"Failed to parse detection data for event at {row['timestamp']}")
    ...
    def create_export_summary(self, export_data: ExportData) -> Dict[str, Any]:
        ...
        goal_achieved_days = sum(1 for stats in export_data.daily_stats if stats.goal_achieved)
        ...
        return {
            ...
            'summary': {
                ...
                'goal_achievement_rate': (goal_achieved_days / total_days * 100) if total_days > 0 else 0,
