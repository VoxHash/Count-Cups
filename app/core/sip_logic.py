...
from app.core.models import DetectionResult, SipEvent, CupProfile, EventSource
...
class SipAggregator:
    ...
    def __init__(
        ...
        # State tracking
        self.sip_start_time: Optional[float] = None
        self.sip_detections: List[DetectionResult] = []
        self.last_sip_time: float = 0.0
        self.is_sip_in_progress = False
    
    def process_detection(self, detection: DetectionResult) -> Optional[SipEvent]:
    ...
    def _complete_sip(self) -> Optional[SipEvent]:
        ...
        avg_confidence = sum(d.confidence for d in self.sip_detections) / len(self.sip_detections)
        avg_head_tilt = sum(d.head_tilt_angle for d in self.sip_detections if d.head_tilt_angle) / len(self.sip_detections)
        avg_hand_distance = sum(d.hand_face_distance for d in self.sip_detections if d.hand_face_distance) / len(self.sip_detections)
        ...
        logger.info(f"Sip event created: {sip_event.ml_estimate:.1f}ml, confidence: {avg_confidence:.2f}")
        return sip_event
...
class CupConverter:
    ...
    def __init__(self, default_profile: Optional[CupProfile] = None):
    ...
    def sips_to_cups(self, sips: int, profile: Optional[CupProfile] = None) -> float:
    ...
    def ml_to_cups(self, ml: float, profile: Optional[CupProfile] = None) -> float:
    ...
    def cups_to_ml(self, cups: float, profile: Optional[CupProfile] = None) -> float:
    ...
    def estimate_sip_ml(self, sip_event: SipEvent, profile: Optional[CupProfile] = None) -> float:
...
class SipTracker:
    ...
    def __init__(
        self,
        sip_aggregator: Optional[SipAggregator] = None,
        cup_converter: Optional[CupConverter] = None
    ):
    ...
    def process_detection(self, detection: DetectionResult, profile: Optional[CupProfile] = None) -> Optional[SipEvent]:
        ...
        if sip_event:
            ...
            if sip_event.ml_estimate <= 0:
                sip_event.ml_estimate = self.cup_converter.estimate_sip_ml(sip_event, profile)
            ...
            logger.info(f"Sip tracked: {sip_event.ml_estimate:.1f}ml (Total today: {self.total_ml_today:.1f}ml)")
        
        return sip_event
    
    def add_manual_sip(self, ml: float, profile: Optional[CupProfile] = None) -> SipEvent:
        ...
        logger.info(f"Manual sip added: {ml:.1f}ml (Total today: {self.total_ml_today:.1f}ml)")
        return sip_event
    ...
    def get_sip_rate(self, window_minutes: int = 60) -> float:
        ...
        recent_sips = [
            event for event in self.sip_events_today
