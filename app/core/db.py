...
from app.core.models import (
    CupProfile, DailyGoal, SipEvent, UserSettings, 
...
class Database:
    ...
    def _migration_001_initial_schema(self, cursor: sqlite3.Cursor) -> None:
        ...
        # Create indexes
        cursor.execute("CREATE INDEX idx_sip_events_timestamp ON sip_events (timestamp)")
        cursor.execute("CREATE INDEX idx_sip_events_profile_id ON sip_events (profile_id)")
    ...
    def create_cup_profile(self, profile: CupProfile) -> int:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            profile.name, profile.size_ml, profile.sips_per_cup, profile.color,
            profile.is_default, profile.created_at, profile.updated_at
        ))
        self.connection.commit()
        return cursor.lastrowid
    
    def get_cup_profile(self, profile_id: int) -> Optional[CupProfile]:
    ...
    def update_cup_profile(self, profile: CupProfile) -> bool:
        ...
        """, (
            profile.name, profile.size_ml, profile.sips_per_cup, profile.color,
            profile.is_default, profile.updated_at, profile.id
        ))
        self.connection.commit()
    ...
    def create_sip_event(self, event: SipEvent) -> int:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            event.timestamp, event.profile_id, event.ml_estimate, str(event.source),
            event.confidence, event.detection_data
        ))
        self.connection.commit()
        return cursor.lastrowid
    ...
    def get_or_create_daily_goal(self, target_date: date, target_ml: int) -> DailyGoal:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            goal.date, goal.target_ml, goal.achieved_ml, goal.is_achieved,
            goal.streak_days, goal.created_at, goal.updated_at
        ))
    ...
    def update_daily_goal(self, goal: DailyGoal) -> bool:
        ...
        cursor.execute("""
            ...
            WHERE id = ?
        """, (
            goal.target_ml, goal.achieved_ml, goal.is_achieved,
            goal.streak_days, goal.updated_at, goal.id
        ))
        self.connection.commit()
        return cursor.rowcount > 0
    ...
    def get_daily_stats(self, target_date: date) -> DailyStats:
        ...
        # Get daily goal
        goal = self.get_or_create_daily_goal(target_date, settings.default_cup_size_ml * 4)  # 4 cups default
        goal_achieved = total_ml >= goal.target_ml
        progress_percentage = (total_ml / goal.target_ml * 100) if goal.target_ml > 0 else 0
    ...
    def get_user_settings(self) -> UserSettings:
        ...
        """, (
            1, settings_obj.theme.value, settings_obj.detection_engine.value,
            settings_obj.enable_notifications, settings_obj.goal_reminder_hour,
            settings_obj.goal_reminder_minute, settings_obj.default_cup_profile_id,
            settings_obj.window_width, settings_obj.window_height, settings_obj.window_maximized,
            settings_obj.camera_index, settings_obj.camera_width, settings_obj.camera_height,
            settings_obj.camera_fps, settings_obj.created_at, settings_obj.updated_at
        ))
        self.connection.commit()
    ...
    def update_user_settings(self, settings_obj: UserSettings) -> bool:
        ...
        theme_value = settings_obj.theme.value if hasattr(settings_obj.theme, 'value') else str(settings_obj.theme)
        engine_value = settings_obj.detection_engine.value if hasattr(settings_obj.detection_engine, 'value') else str(settings_obj.detection_engine)
        ...
        cursor.execute("""
            ...
            WHERE id = ?
        """, (
            theme_value, engine_value,
            settings_obj.enable_notifications, settings_obj.goal_reminder_hour,
            settings_obj.goal_reminder_minute, settings_obj.default_cup_profile_id,
            settings_obj.window_width, settings_obj.window_height, settings_obj.window_maximized,
            settings_obj.camera_index, settings_obj.camera_width, settings_obj.camera_height,
            settings_obj.camera_fps, settings_obj.updated_at, settings_obj.id
        ))
        self.connection.commit()
        return cursor.rowcount > 0
