...
class Scheduler:
    ...
    def _should_run_task(self, task: dict, current_time: datetime) -> bool:
        ...
        if task['type'] == 'once':
            ...
            return (current_time.time() >= task['run_time'] and 
                    current_time.date() > task['last_run'])
        elif task['type'] == 'interval':
            return (current_time - task['last_run']).total_seconds() >= task['interval']
        
        return False
    
    def schedule_once(self, callback: Callable, run_time: datetime, task_id: Optional[str] = None) -> str:
    ...
    def schedule_daily(self, callback: Callable, run_time: dt_time, task_id: Optional[str] = None) -> str:
    ...
    def schedule_interval(self, callback: Callable, interval_seconds: int, task_id: Optional[str] = None) -> str:
